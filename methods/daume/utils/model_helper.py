    ...
    def fit(self, dataset, y=None):
        ...
        if not self.use_large:
            for key in self.uniq_domains:
                ...
                self.tfidf_vec_da[key] = TfidfVectorizer(ngram_range=(1, 3), min_df=2, max_features=15000)
                new_docs = []
                for item in dataset:
                    if item[self.column_idx] == key:
                        new_docs.append(item[1])
                self.tfidf_vec_da[key].fit(new_docs)
            self.tfidf_vec_da["general"] = TfidfVectorizer(ngram_range=(1, 3), min_df=2, max_features=15000)
            ...
            for key in self.tfidf_vec_da:
                ...
                tmp_vect = TfidfVectorizer(min_df=3, tokenizer=self.da_tokenizer, max_features=15000)
                tmp_vect.fit([item[1] for item in dataset if item[self.column_idx] == key])
            ...
            tmp_vect.fit([item[1] for item in dataset])
        ...
        if not self.use_large:
            for domain in self.uniq_domains:
                tmp_fvs = csc_matrix(self.tfidf_vec_da[domain].transform(
                    [item[1] if item[self.column_idx] == domain else "" for item in dataset]
                ))
...
def build_lr_clf(init_settings=None):
    ...
    return SGDClassifier(
        loss='log', penalty='elasticnet', class_weight='balanced', n_jobs=-1, l1_ratio= init_settings['l1_ratio'], max_iter=init_settings['max_iter'], alpha=0.0001*init_settings['C'], tol=init_settings['tol']
    ) # l1_ratio= init_settings['l1_ratio'], max_iter=init_settings['max_iter'], alpha=0.0001*init_settings['C'], tol=init_settings['tol']
    ...
    def fit(self, dataset, y=None):
        ...
        if not self.use_large:
            for key in self.tfidf_vec_da:
                ...
                self.tfidf_vec_da[key].fit([item[1] for item in dataset if item[self.column_idx] == key])
            ...
            for key in self.tfidf_vec_da:
                ...
                self.tfidf_vec_da[key] = \
                    ...
                    + str(key) + '_binary.pkl'
                tmp_vect = TfidfVectorizer(ngram_range=(1,3), min_df=2, binary=True, 
                    token_pattern=None, max_features=150000,
                    tokenizer = self.da_tokenizer, use_idf=False, smooth_idf=False)
                tmp_vect.fit([item[0] for item in dataset if item[1] == key])
                pickle.dump(tmp_vect, open(self.tfidf_vec_da[key], 'wb'))

            tmp_vect = TfidfVectorizer(ngram_range=(1,3), min_df=2, binary=True, 
                    use_idf=False, max_features=150000,
