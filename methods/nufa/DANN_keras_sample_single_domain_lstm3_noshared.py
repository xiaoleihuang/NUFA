"""
    ...
    2 BiLSTMs, no shared version, one for domain, one for classification
"""
...
def run_dnn(data_name, dkeys):
    ...
    # Bi-LSTM
    bilstm_senti = Bidirectional(LSTM(200, dropout=0.2), name='senti_lstm')
    bilstm_domain = Bidirectional(LSTM(200, dropout=0.2), name='domain_lstm')
    ...
    dense_1 = Dense(128, activation='relu')(lstm_senti)
    ...
    for dkey in dkeys:
        ...
        domain_inputs[dkey].append(
            bilstm_domain(domain_inputs[dkey][-1]) # embedding as input
        )
        domain_inputs[dkey].append(
    ...
    for e in range(epoch_num):
        ...
        if cur_dev > best_dev:
            ...
            test_result = open('./DANN_keras_sample_single_domain_lstm3_noshared.txt', 'a')
